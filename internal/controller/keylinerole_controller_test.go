...
var _ = Describe("KeylineRole Controller", func() {
	const (
		roleName  = "test-role"
		projName  = "test-role-proj"
		vsName    = "test-role-vs"
		instName  = "test-role-instance"
		namespace = "default"
	)
	...
	newReconciler := func() *KeylineRoleReconciler {
		return &KeylineRoleReconciler{
			Client: k8sClient,
	...
	AfterEach(func() {
		cleanup := []struct {
			...
			{roleNamespacedName, &keylinev1alpha1.KeylineRole{}},
	...
	newRole := func(projRef string) *keylinev1alpha1.KeylineRole {
	...
	assertNotReady := func(reason string) {
		role := &keylinev1alpha1.KeylineRole{}
		Expect(k8sClient.Get(ctx, roleNamespacedName, role)).To(Succeed())
		cond := meta.FindStatusCondition(role.Status.Conditions, keylinev1alpha1.ConditionReady)
	...
	Context("when the referenced KeylineProject does not exist", func() {
		BeforeEach(func() {
			Expect(k8sClient.Create(ctx, newRole("nonexistent-proj"))).To(Succeed())
		})

		It("sets Ready=False with reason ProjectNotFound", func() {
			_, err := newReconciler().Reconcile(ctx, reconcile.Request{NamespacedName: roleNamespacedName})
