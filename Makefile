...
.PHONY: hooks
hooks: ## Install git hooks from .githooks/ into the local repo.
...
.PHONY: test-e2e
test-e2e: manifests generate fmt vet ## Run the e2e tests against a local minikube cluster.
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name keyline-operator-builder
	$(CONTAINER_TOOL) buildx use keyline-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm keyline-operator-builder
	rm Dockerfile.cross
